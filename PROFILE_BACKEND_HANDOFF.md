...
- หลังรวม `main` ที่มีงาน cart/checkout: หน้า Checkout ยังจำลองการไปหน้า confirmation โดยไม่เรียก `POST /orders` และ CartPage ใช้ `localStorage` ขณะที่ My Orders อ่านจากฐานข้อมูล จึงยังไม่เห็นคำสั่งซื้อที่กดจาก Checkout ในประวัติจริง งานเชื่อมตะกร้าและสร้าง order ต้องทำต่อก่อนใช้ flow นี้จริง
