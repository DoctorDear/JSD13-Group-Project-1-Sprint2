export default function FormError({ message, className = "" }) {
  ...
  return (
    <div
      role="alert"
      aria-live="assertive"
      className={`flex items-start gap-3 rounded-lg border border-red-300 bg-red-50 px-4 py-3 ${className}`}
    >
      ...
      <p className="text-[11px] font-medium text-red-700">{message}</p>
    </div>
  );
}